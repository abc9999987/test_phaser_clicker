...
// 젬 던전 씬
export class GemDungeonScene extends BaseDungeonScene {
    ...
    protected initializeDungeon(): void {
        const dungeonConfig = DungeonConfigs.find(d => d.id === 'gem_dungeon');
        if (!dungeonConfig) {
            console.error('젬 던전 설정을 찾을 수 없습니다.');
            return;
        }
    ...
    startDungeonTimer(): void {
        ...
        this.dungeonTimer = this.time.addEvent({
            delay: dungeonConfig.timeLimit * 1000,
            callback: () => {
                this.onTimerExpired();
            },
            loop: false
