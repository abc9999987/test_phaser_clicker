...
export const UIManager = {
    ...
    create(scene: Phaser.Scene): void {
        ...
        // 네 번째 탭 (Dungeon) 내용 생성
        this.createDungeonTab(scene, gameWidth, gameHeight, halfHeight, uiAreaHeight, uiAreaStartY, 3);
    ...
    // Dungeon 탭 내용 생성
    createDungeonTab(scene: Phaser.Scene, gameWidth: number, _gameHeight: number, _halfHeight: number, uiAreaHeight: number, uiAreaStartY: number, tabIndex: number): void {
        // 기존 탭 컨텐츠 제거
        if (this.tabContents[tabIndex]) {
            this.tabContents[tabIndex].destroy();
            this.tabContents[tabIndex] = null as any;
        ...
        // 기존 던전 카드들 제거
        this.dungeonCards.forEach(card => card.destroy());
        ...
        DungeonConfigs.forEach((dungeonConfig, index) => {
            ...
            const dungeonCard = this.createDungeonCard(scene, dungeonConfig, cardX, cardY, dungeonCardWidth, dungeonCardHeight);
            contentContainer.add(dungeonCard);
            this.dungeonCards.push(dungeonCard);
        });
